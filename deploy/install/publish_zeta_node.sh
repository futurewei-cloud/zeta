...
echo "Rebuild and publish zetanode image to $REG..."
...
echo "Rebuild and publish zeta-droplet image to $REG..."
docker image build -t $REG/zeta-droplet:latest -f ${ROOT}/deploy/kind/droplet.Dockerfile $ROOT >/dev/null
docker image push $REG/zetanode:latest >/dev/null
