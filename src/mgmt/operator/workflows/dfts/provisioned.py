...
def dft_provisioned(task, dft, name, body, spec, diff):
    ...
    for d in diff:
        logger.info("diff_field:{}, from:{}, to:{}".format(d[1], d[2], d[3]))
        if d[0] == 'change':
...
def process_change(dft, field, old, new, task):
    if field[0] == 'spec' and field[1] == 'numchains':
        ...
        return chains_opr.process_numchain_change(dft, int(old), int(new), task)
    elif field[0] == 'spec' and field[1] == 'numfwds':
        logger.info("diff_field:{}, from:{}, to:{}".format(field, old, new))
        return fwds_opr.process_numfwd_change(dft, int(old), int(new), task)
